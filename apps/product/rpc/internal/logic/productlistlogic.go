...
func (l *ProductListLogic) ProductList(in *product.ProductListRequest) (*product.ProductListResponse, error) {
	...
	if len(pids) == int(in.Ps) {
		...
		products, err = l.svcCtx.ProductModel.CategoryProducts(l.ctx, ctime, int64(in.CategoryId), int64(in.Ps))
		if err != nil {
			return nil, err
		}
...
func (l *ProductListLogic) productsByIds(ctx context.Context, ids []int64) ([]*model.Product, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	products, err := mr.MapReduce[any, any, any](func(source chan<- interface{}) {
		for _, pid := range ids {
			source <- pid
		}
...
func categoryKey(cid int32) string {
	return fmt.Sprintf("%s%v", "cache:product:category:id:", cid)
}
